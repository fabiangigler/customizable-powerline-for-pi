...
import { getThemeNames, getThemePaths, sanitizeThemeName } from "./utils.ts";
...
const parsePublishArgs = (rawArgs: string) => {
  const parts = rawArgs.trim().split(/\s+/).filter(Boolean);
  const force = parts.includes("--force") || parts.includes("-f");
  const themeName = sanitizeThemeName(
    parts.find((part) => part !== "--force" && part !== "-f") ?? "",
  );
  return { force, themeName };
};
...
const executePowerlineCommand = async (
  ...
  if (command === "publish") {
    const publishArgs = parsePublishArgs(rawName);
    try {
      const filePath = publishDefaultConfig(
        ctx.cwd,
        publishArgs.themeName && publishArgs.themeName !== "default"
          ? publishArgs.themeName
          : undefined,
        { force: publishArgs.force },
      );
      ctx.ui.notify(`Published powerline config: ${filePath}`, "success");
    } catch (error) {
      ctx.ui.notify(
        `${error instanceof Error ? error.message : String(error)}. Use /powerline:publish --force to overwrite.`,
        "warning",
      );
    }
    return;
  ...
  if (command === "theme") {
    if (
      themeName &&
      themeName !== "default" &&
      !getThemePaths(ctx.cwd, themeName).some((path) => existsSync(path))
    ) {
      ctx.ui.notify(
  ...
  ctx.ui.notify(
    "Usage: /powerline, /powerline:status, /powerline:publish [--force] [theme], or /powerline:theme [theme]",
    "warning",
  );
};
...
export const registerPowerlineCommand = (
  ...
  const aliases: Array<{ name: string; command: string; description: string }> = [
    ...
    { name: "powerline:publish", command: "publish", description: "Publish editable powerline config/theme; add --force to overwrite" },
